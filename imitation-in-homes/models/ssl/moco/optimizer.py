...
class MoCoScheduler(torch.optim.lr_scheduler._LRScheduler):
    def __init__(
        self,
        optimizer,
        warmup_epochs: int,
    ...
    def get_lr(self):
        if self.last_epoch < self.warmup_epochs:
            lr = (
                self.start_lr
                + (self.base_lr - self.start_lr) * self.last_epoch / self.warmup_epochs
            )
        else:
            lr = self.end_lr + (self.base_lr - self.end_lr) * 0.5 * (
                1.0
                + math.cos(
                    math.pi
